#define NinebotPort Serial1                                        // Порт подключения к самокату
#define ESC 0x20                                                   // Контролер
#define BLE 0x21                                                   // Приборка с блютус
#define BMS 0x22                                                   // Контролер батареи
#define IOT 0x3D                                                   // Приложение?
#define APP 0x3E                                                   // Приложение
#define timeoutQuery 100      // пауза между запрсами, мс                                     
...
byte req0[] = {0x5A, 0xA5, 0x01, 0x3E, 0x20, 0x01, 0xB0, 0x20, 0xCF, 0xFE};    //indexQuery 0 скорость, время, пробег и т.д.
byte req1[] = {0x5A, 0xA5, 0x01, 0x3E, 0x22, 0x01, 0x31, 0x0A, 0x62, 0xFF};    //indexQuery 1 батарея: напряж ток % емк
byte req2[] = {0x5A, 0xA5, 0x01, 0x3E, 0x22, 0x01, 0x40, 0x10, 0x4D, 0xFF};    //indexQuery 2 батарея: первые 8 банок
byte req3[] = {0x5A, 0xA5, 0x01, 0x3E, 0x22, 0x01, 0x48, 0x10, 0x45, 0xFF};    //indexQuery 3 батарея: последние две банки
long timer;
...
void setup() {
  ...
  NinebotPort.begin(115200);                                           // включаем и настраиваем последовательный порт к скутеру
}
...
void Unpack() {                 // процедура распаковки пакета
  ...
  if (data[3] == ESC && data[4] == APP && data[5] == 0x04 && data[6] == 0xB0) { // если пришел пакет B0
  ...
  if ((data[3] == BLE && data[4] == ESC) || (data[3] == ESC && data[4] == APP)) { // если пришел пакет
...
void Query(byte indexQuery) {                                                //процедура отправки запрса
  ...
  switch (indexQuery) {
    case 0:
      NinebotPort.write(req0, sizeof(req0));
      break;
    case 1:
      NinebotPort.write(req1, sizeof(req0));
      break;
    case 2:
      NinebotPort.write(req2, sizeof(req0));
      break;
    case 3:
      NinebotPort.write(req3, sizeof(req0));
      break;
  }
  UCSR1B |= _BV(RXEN1);
}
...
void ReceivingData() {                                        //процедура получения данных
  if (millis() - timer > timeoutQuery ) {
    Query(0);
    timer = millis();
  }
  if (NinebotPort.available()) {                                       // если в порту есть данные
    c = NinebotPort.read();                                            // читаем из порта байт и он стирается из буфера
    switch (state) {
      case 0:                                                      // если находимся в состоянии 0 поиска заголовка
        ...
        break;                                                     // выходим из case и возвращемся в начало loop
      ...
      case 2:                                                      // если находимся в состоянии 2
        if (curs < 2 || (curs >= 2 && curs < (9 + data[2]))) {     // если текущий элемент массива меньше 2 или больше 2 но меньше 9 + третий элемент массива (размер данных из пакета)
          ...
          state = 3;                                               // переходим в состояние 3 - вывод результатов
        }
        break;                                                     // выходим из case и возвращемся в начало loop
